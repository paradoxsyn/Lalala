...
public class GameState extends State {
    ...
    private long crystalstotal;

    private boolean stopCol = false;
    private boolean jumped = false;
    private boolean tapcancel,tapcancel2,tapcancel3,tapcancel4 = false;
    private boolean flinger = false;
    private boolean stoploaderact = false;
    private boolean speeduponce = false;
    ...
    public GameState(StateManager manager, float speed, int level){
        super(manager);
       ...
       // overload = new com.badlogic.gdx.scenes.scene2d.ui.Image(manager.game.manager.get("overloadpng.png",Texture.class));
        ...
        bg = new com.badlogic.gdx.scenes.scene2d.ui.Image(Assets.ryzeflip);
        bg2 = new com.badlogic.gdx.scenes.scene2d.ui.Image(Assets.ryzeflip);
        bg3 = new com.badlogic.gdx.scenes.scene2d.ui.Image(Assets.ryzeflip);
        bg4 = new com.badlogic.gdx.scenes.scene2d.ui.Image(Assets.ryzeflip);
        ...
        tahm = new TahmK(w/2-w/8-w/5, screenBounds.getHeight()-200,Align.center,200,200);
        ...
        overload.setOrigin(overload.getImageWidth()/2,overload.getImageHeight()/2);
    ...
    public void runeInput(){
        runepris.addListener(new DragListener() {
            ...
            public void touchDragged (InputEvent event, float x, float y, int pointer) {
                // example code below for origin and position
                //overload.texture2.setOrigin(Gdx.input.getX(), Gdx.input.getY());
                //overload.setPosition(x, y);
    ...
    public void update(float dt) {
        ...
        else if(overload.getY() > screenBounds.getHeight()-150){
            if(!playonce){
        ...
        else if(overload.getY() > 400 & flingtime > 4 && overload.getY() < tahm.actorTahm().getY()){
            if(!playonce){
                stage.addAction(Actions.fadeOut(3f));
                overload.clearActions();
                tahm.actorTahm().clearActions();
    ...
    public void nextPhase(){
        ...
        if(num==0){
            ...
            moveleft.setPosition(screenBounds.getX()+100, screenBounds.getHeight()-200, Align.center);
            moveleft.setDuration(1.5f);

            MoveToAction moveright = new MoveToAction();
            moveright.setPosition(screenBounds.getWidth()+50, screenBounds.getHeight()-200,Align.center);
            moveright.setDuration(1.5f);

            SequenceAction seq = new SequenceAction(Actions.parallel(moveleft,Actions.scaleBy(-.5f,0,1.5f,Interpolation.exp10In)),Actions.scaleBy(.5f,0,.4f,Interpolation.exp10In),Actions.parallel(moveright,Actions.scaleBy(-.5f,0,1.5f,Interpolation.exp10In)));
            seq.addAction(Actions.scaleBy(.5f,0,.4f,Interpolation.exp10In));
    ...
    public int getRandArrayElement(int[] spot){
        int rnd = new Random().nextInt(spot.length);
        return spot[rnd];
    }

    public float getProg(float load, float start){
        load += ((counter - tofull) / tofull);
        return Math.min(1, load / start);
    }
    ...
    public void fizzle(){
        ...
        eye1.addAction(Actions.sequence(Actions.fadeOut(3f),Actions.removeActor()));
        eye2.addAction(Actions.sequence(Actions.fadeOut(3f),Actions.removeActor()));
    ...
    public void tappedOut(){
        ...
        if(counter>=450f){
            ...
            eye5.addAction(Actions.sequence(Actions.fadeOut(3f),Actions.removeActor()));
            eye6.addAction(Actions.sequence(Actions.fadeOut(3f),Actions.removeActor()));
            if(elapsedTime<=10){
                if(!scorestop5) {
    ...
    public void progress(){
        ...
        */
        ...
        //loader.getAnimation().setPlayMode(Animation.PlayMode.NORMAL);
    ...
    public void bouncefinish(){
        if(bouncefinished && bouncefinished2 && bouncefinished3 && bouncefinished4){
            ...
            eye7.addAction(Actions.sequence(Actions.fadeOut(1.5f),Actions.removeActor()));
            eye8.addAction(Actions.sequence(Actions.fadeOut(1.5f),Actions.removeActor()));
            ...
            if(hit1&&hit2&&hit3&&hit4){
                prefs.putBoolean("unlocklevel2",true);
                crystalstotal = crystals+crystals2+crystals3+crystals4;
                prefs.putLong("crystalstotal",crystalstotal);
            }
        }
    }
